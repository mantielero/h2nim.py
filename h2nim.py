...
def add_header_and_libname(headerFileName, libFileName):
    ...
    # Macros
    tmp = []
    tmp.append( (f"header{name}", f"\"{headerFileName}\"", True,1) )
    tmp.append( (f"libName", f"\"{libFileName}\"", False,1) )
...
def parse_macros( data, parser ):
    ...
    for k,v in parser.defs["macros"].items():
        ...
        if val != None:
            tmp.append( (k, val, False,1) )
        else:
            tmp.append( (k, '""', False,1) )
    _old = data.get("const", [])
    data["const"] = _old + tmp     
    return data
...
def parse_enums( data, parser ):
    _tmp = {"top" : [("import nimterop/types   # Provides \"defineEnum\"",None, True, 0)]}
    ...
    for k,v in parser.defs["enums"].items():
        enumlist.append(k)
        _tmp["top"].append((f"defineEnum({k})", None, False,0))
        _tmp["top"][2] = False # Don't comment
        for k1,v1 in v.items():
            tmp.append( (f"{k1}", f"({v1}).{k}", False, 2) )
        tmp.append( (None, None, False,0))  # Add a space


    for k, v in parser.defs["types"].items():
        if v.type_spec.startswith("enum "):
            if not k in enumlist:
                _tmp["top"].append( f"defineEnum({k})" )
                _tmp["top"][2] = False                
                ...
                for i in range(len(_tmp)//2):
                    tmp.append( (f"  {_tmp[i*2]}", "({_tmp[i*2+1]}).{k}", False) ) 
                tmp.append( (None, None, False))  # Add a space    

    _old = data.get("top", [])
    data["top"] = _old + _tmp["top"]  
...
def create_types(data, parser):
    ...
    for k,v in parser.defs["structs"].items():
        ...
        _list.append((f"{_object_name}", "object", False, 1) )  #importc: \"struct {k}\", header: header{name},
        ...
        for member in v.members:
            ...
            if len(params) == 0:
                ...
                _list.append((f"{_name}",f"{_new_type}", False, 2) )
            ...
            # Function signatures declarations
                ...
                if ret_type == "void":
                    _list.append( (f"{fname}", f"proc({_parameters}) {{.cdecl.}}", False, 2) )
                else:
                    _list.append( (f"{fname}", f"proc({_parameters}):{ret_type} {{.cdecl.}}", False, 2) )


        _list.append((None, None, False, 0))
    _old = data.get("type", [])
    data["type"] = _old +_list
    return data

def gen_new_function(name, fname, params,ret_type):
    new = "proc("
    add_comma = False
    _parameters = ""
    for t in params:
    ...
    _tmp = ("fname", new, False, 2 )
    return _tmp


def create_something(data, parser):
    _tmp = []
    for k, v in parser.defs["types"].items():
...
def create_text(data, tab = 2):
    # Top + defineEnums
    _txt = ""
    for txt1, txt2, comment, level in data["top"]:
        if not comment:
            _txt += " "*level*tab + txt1 + "\n\n\n" 
    
    # Consts
    _txt += "const\n"
    for txt1, txt2, comment, level in data["const"]:
        spaces = level * tab * " "
        sep = "="
        if level > 1:
            sep = ":"
    ...
    # Types
    _txt += "type\n"
    for txt1, txt2, comment, level in data["type"]:
        spaces = level * tab * " "
        if not comment:
            if level == 0 and txt1 == None and txt2 == None:
                _txt += "\n"
                ...
                _txt += spaces +  f"{txt1}* {{.bycopy.}} = {txt2}\n"
            elif level == 2:
                _txt += spaces +  f"{txt1}* : {txt2}\n"
    
    # Functions
    _txt += "{.push dynlib:libName,importc.}\n"
    for txt1, txt2, comment, level in data["functions"]:
        if not comment:
            _txt += f"{txt1}\n"
    _txt += "{.pop.}\n"
    return _txt
...
if __name__ == "__main__":
    ...
    data = create_something(data, parser) 
